.. autocsr documentation master file, created by
   sphinx-quickstart on Wed Apr 21 18:26:42 2021.
...
generating Certificate Signing Requests from basic user input and easy
...
Requests with minimal security knowledge required.



``AutoCSR`` also provides security professionals with the ability to

define complex Certificate Signing Requests with templates that can be

easily shared with non-security professionals to generate complex

Certificate Signing Requests without the need for detailed instructions
...
The easiest way to generate a Certificate Signing Request is by

specifying the common name and letting ``autocsr create`` handle the

rest! This is perfect for simple Certificate Signing Requests that don�t

require any custom attributes, extensions, or specific key formats.
...
You can also specify the ``key-path`` and ``output-path`` to avoid the
...
If you need to generate a Certificate Signing Request without custom

attributes or extensions, but still want to customize the subject and

signing information, ``autocsr prompt`` can guide you through the

process while offering you safe defaults and a list of options where
...
For more customized Certificate Signing Requests and those that need to

be repeatedly generated in a consistent way, it will often be desirable

to add the details of your Certificate Signing Request inside a

template. ``autocsr build`` can be used to construct one or many

Certificate Signing Requests from the details present in a template.
...
A template is an overall definition of the data you want to be include

in your Certificate Signing Request. Most fields can be optionally

excluded or have safe default values for those who don�t want to worry
...
The subject of the Certificate Signing Request contains various metadata

about the certificate, the only required portion being the
...
``AutoCSR`` allows keys to be generated on the fly or loaded directly

from files. You can also explicitly define the hash algorithm to use for
...
Running ``autocsr`` on this template will generate three Certificate
...
use YAML for our template, we require that attribute values are base64

encoded in the template when defining attributes. Optionally for

predefined attributes, a string name can be used instead of the
...
but I will provide a few examples of modeling extensions in templates

below. Keep in mind that like attributes, extensions that require bytes

as input will need to have their data represented in base64.
...
Alternatively to using ``.yaml`` files, you can use ``.jinja2`` files

modeled after YAML, but including environment variables to be replaced.

If you wanted to recreate the first example using environment variables,
